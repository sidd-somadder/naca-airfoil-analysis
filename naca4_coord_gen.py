"""
From-scratch script for NACA 4-series airfoil generation. Uses known NACA 4-digit equations for mean camberline & thickness distribution.
DOES NOT CLOSE TRAILING EDGE FOR COMPUTATION: open trailing edge is handled in constant_vpm.py 
The geometric data points of the airfoil are exported in (x,y) coordinates in a .dat file in saved_coordinates folder
Run this script to save coordinate .dat file with exact name convention used in airfoil_aerodynamic_analysis.py script.
Follow instructions on terminal: Expected 4 digit airfoils ONLY. 
"""
...
def naca_plot(NACA_dig, XY_coords):
    ...
    plt.xlabel('x/c')
    plt.ylabel('y/c')
    plt.title(f'NACA {code} Airfoil Geometry (Half-Cosine Spacing, {panel_ct} panels)')
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    print("---")
...
def get_mesh_param():
    while True:
        ...
        print(f"User Note: {N} chordwise points will be used for computation; "
              f"this generates {panel_ct} total surface points.")
